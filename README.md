# Gilbert Font | (Holloway) Chew, Kean Ho's Creative Visuals

[![banner](/Pictures/banner_1200x630.svg)](#)

[![banner](/Pictures/banner-colored_1200x630.svg)](#)

Gilbert is a Latin (ID: `*Latn`) and Japanese Katakana (ID: `Ja` (Kata)) display
font designed by
[The TypeWithPride Project Team](https://github.com/Fontself/TypeWithPride).
It was made for honoring the late Gilbert Baker, the creator of LGBTQ iconic
Rainbow Flag. This font has 2 offering:

* Plain color mode (`Gilbert`) - the plain monochrome colored text where the
  editor set  the color.
* Pride color mode (`Gilbert Color`) - the automated coloration of text with the
  Pride rainbow colors. The colorations are randomized.

Note that for `Gilbert Color`, it is near impossible to convert the text
verbatim into path object in image editor like Inkscape. They will turn into
the plain color mode instead. Hence, **this means the font can ONLY be used in
text form** so you need to re-distribute the font files downstream for it to
work.

The source repository is available at: https://github.com/Fontself/TypeWithPride

> [!CAUTION]
>
> Unlike other re-distributed fonts, this font retains its original license
> which is
> [Creative Commons Attribution-ShareAlike 4.0 International License](LICENSE.txt).
> It **REQUIRES the source (original or modified) to be always available
> downstream**. Hence, **BE CAREFUL** before using this font especially in
> non-open commercial projects (as this license will force it open).

This font is ideal for any LGBTQ related typefaces like event banner, content
title, page headings, etc. Due to the license's requirements, it is best to
only use the font in short-term open resources projects and events where the
general public has access to the source files.
...
3. All fonts **MUST be strictly licensed under
   ...
   without any private arbitrary terms (e.g. people from country X are
   prohibited to use, not allowed to re-distribute, trademarking font, etc)**.
...
Attribution is **COMPULSORY**. You **MUST** attribute back to the creator(s) as
follows:

```
Title: Gilbert Font
Creators: The TypeWithPride Project Team
Packaged-By: (Holloway) Chew, Kean Ho
Contact: hello@chewkeanho.com
SKU: chewkeanho-visuals-fonts-gilbert
UUID: 263657E9-5DAD-4FD9-8287-1B87C351D40B
License: Creative Common Attribution-ShareAlike 4.0 International (http://creativecommons.org/licenses/by-sa/4.0)
Repository Made On: 2026-03-28
Repository Made From: Malaysia, South East Asia
Procure: https://github.com/ChewKeanHo/visuals-fonts-gilbert
```
...
1. Providing proper attribution (see above); AND
2. Retaining the same license downstream for the font files.
...
1. Providing proper attribution (see above); AND
2. Retaining the same license downstream for the font files; AND
3. You may use the font files in commercial works.
...
1. Providing proper attribution (see above); AND
2. Retaining the same license downstream for the font files; AND
3. Making the source of your modifications available (e.g., editable svg files)
   and reproducible without any impedance downstream.
...
2. Retaining the same license downstream for the font files; AND
3. Making the source of your modifications available (e.g., editable svg files)
   and reproducible without any impedance downstream; AND
4. You may sell the output of your remix (e.g., a design, video, or product).
...
1. Providing proper attribution (see above); AND
2. Retaining the same license downstream for the font files; AND
3. If you distribute the modified versions, you **MUST** make their source
   (e.g., editable svg files) available and reproducible without any impedance
   downstream.
...
3. If you distribute the modified versions, you **MUST** make their source
   (e.g., editable svg files) available and reproducible without any impedance
   downstream; AND
4. You may charge a fee for access, but you **MUST** also provide the
   corresponding source (for modifications) and clearly state the license terms.
